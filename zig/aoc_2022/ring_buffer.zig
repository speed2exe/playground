...
test "test readFrom" {
    var data = StringReader.init("0123456789");
    var reader = data.reader();
    var ring_buffer = RingBuffer(8).init();

    {
        const n = try ring_buffer.readFrom(StringReader.Reader, reader);
        try testing.expect(n == 8);
...
test "test writeTo" {
    ...
    _ = try ring_buffer.readFrom(StringReader.Reader, reader);
...
test "test writeTo - 2" {
    // populate
    var data = StringReader.init("abcdefghij");
    var reader = data.reader();
    var ring_buffer = RingBuffer(10).init();
    _ = try ring_buffer.readFrom(StringReader.Reader, reader);
    ring_buffer.head = 7;
