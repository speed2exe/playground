...
void renderText(FT_Face face, const char *text, GLfloat x, GLfloat y,
  ...
  for (const char *p = text; *p; p++) {
    ...
    GLfloat ypos =
        y - (face->glyph->bitmap.rows - face->glyph->bitmap_top) * scale;
...
int main() {
  ...
  GLFWwindow *window =
      glfwCreateWindow(800, 600, "OpenGL Text Rendering", NULL, NULL);
  if (window == NULL) {
  ...
  FT_Face face;
  if (FT_New_Face(ft, "/home/zack2827/Downloads/font/IosevkaFixed-Regular.ttf",
                  0, &face)) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    renderText(face, "Hello World", 25.0f, 25.0f, 1.0f,
               (vec3){0.5, 0.8f, 0.2f});
